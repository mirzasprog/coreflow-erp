...
export default function TransferList() {
  ...
  const { data: documents, isLoading } = useWarehouseDocuments('transfer');
  ...
  return (
    <div>
      <Header title="Warehouse Transfer" subtitle="Međuskladišnica • Transfer between warehouses" />
                ...
                {filteredDocuments?.length === 0 ? (
                  ...
                  filteredDocuments?.map((doc) => (
                    <TableRow key={doc.id}>
                      ...
                      <TableCell>{doc.target_locations?.name || '-'}</TableCell>
                      ...
                      <TableCell>
                        <div className="flex gap-1">
                          <Button
                            variant="ghost"
                            size="icon"
                            onClick={() => navigate(`/warehouse/transfers/${doc.id}`)}
                          >
                          ...
                          {doc.status === 'draft' && (
                            <>
                              <Button
                                variant="ghost"
                                size="icon"
                                onClick={() => navigate(`/warehouse/transfers/${doc.id}/edit`)}
                              >
