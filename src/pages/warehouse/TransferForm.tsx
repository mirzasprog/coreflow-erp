...
import { useLocations, useItems } from '@/hooks/useMasterData';
import {
...
export default function TransferForm() {
  ...
  useEffect(() => {
    if (existingDoc) {
      setFormData({
        ...
        target_location_id: existingDoc.target_location_id || '',
      ...
      setFormData(prev => ({
        ...prev,
        document_number: `MS-${Date.now().toString().slice(-6)}`
      }));
    }
  }, [existingDoc, isEdit]);
  ...
  const handleSave = async (shouldPost = false) => {
    ...
    if (!formData.target_location_id) {
      toast.error('Please select target warehouse');
      return;
    }
    if (formData.location_id === formData.target_location_id) {
      toast.error('Source and target warehouses must be different');
      return;
    }
    if (lines.length === 0) {
    ...
    try {
      if (isEdit) {
        ...
        if (shouldPost) {
          await postDocument.mutateAsync({ id, documentType: 'transfer' });
        }
      } else {
        const result = await createDocument.mutateAsync({ document, lines });
        if (shouldPost && result?.id) {
          await postDocument.mutateAsync({ id: result.id, documentType: 'transfer' });
        }
      }
  ...
  return (
    <div>
      <Header 
        title={isEdit ? `Edit Transfer ${formData.document_number}` : 'New Transfer'} 
        subtitle="Međuskladišnica • Transfer between warehouses" 
      />
