...
export default function GoodsIssueForm() {
  ...
  useEffect(() => {
    if (existingDoc) {
      ...
      setFormData(prev => ({
        ...prev,
        document_number: `IZ-${Date.now().toString().slice(-6)}`
      }));
    }
  }, [existingDoc, isEdit]);
  ...
  const handleSave = async (shouldPost = false) => {
    ...
    try {
      if (isEdit) {
        ...
        if (shouldPost) {
          await postDocument.mutateAsync({ id, documentType: 'goods_issue' });
        }
      } else {
  ...
  return (
    <div>
      <Header 
        title={isEdit ? `Edit Issue ${formData.document_number}` : 'New Goods Issue'} 
        subtitle="Izdatnica • Create outgoing goods document" 
      />
