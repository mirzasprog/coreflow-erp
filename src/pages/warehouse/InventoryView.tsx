...
export default function InventoryView() {
  ...
  return (
    <div>
      <Header 
        title={`Inventory ${document.document_number}`} 
        subtitle="Inventura • Physical stock count document" 
      />
        ...
        <div className="mb-4 flex items-center justify-between">
          ...
          <div className="flex gap-2">
            {document.status === 'draft' && (
              <>
                <Button variant="outline" onClick={() => navigate(`/warehouse/inventory/${id}/edit`)}>
            ...
            <CardContent>
              <div className="space-y-2">
                ...
                <div className="flex justify-between text-lg">
                  ...
                  <span className={`font-bold ${document.total_value >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                    €{document.total_value.toFixed(2)}
                  </span>
                </div>
              </div>
            </CardContent>
                ...
                {document.lines?.length === 0 ? (
                  ...
                  document.lines?.map((line, index) => (
                    <TableRow key={index}>
                      ...
                      <TableCell className="text-right">{line.counted_quantity ?? line.quantity}</TableCell>
                      <TableCell className={`text-right ${(line.difference_quantity || 0) >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                        {(line.difference_quantity || 0) > 0 ? '+' : ''}{line.difference_quantity || 0}
                      </TableCell>
                      <TableCell className="text-right">€{line.unit_price.toFixed(2)}</TableCell>
                      <TableCell className={`text-right ${line.total_price >= 0 ? 'text-green-600' : 'text-red-600'}`}>
