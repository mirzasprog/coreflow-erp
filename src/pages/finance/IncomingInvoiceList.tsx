...
export default function IncomingInvoiceList() {
  const { data: invoices, isLoading } = useInvoices('incoming');
  ...
  return (
    <div>
      <Header title="Incoming Invoices" subtitle="Ulazne fakture • Purchase invoices" />
                ...
                {!invoices?.length ? (
                  ...
                  invoices.map((inv) => (
                    <TableRow key={inv.id}>
                      ...
                      <TableCell>
                        <NavLink to={`/finance/invoices/incoming/${inv.id}`}>
